...
module.exports = {
    updateStudentDetails: function (pool, id, mobile_no) {
        return new Promise(function (resolve, reject) {
            ...
            var query = "UPDATE tn_student_list SET " +
                        " sl_roll_number = ? "+
                        " WHERE " +
                        " id = ? AND sl_contact_number  = ?";
    ...
    addNewStudent(pool,sd){
        return new Promise(function(resolve,reject){
            ...
            var query =  "INSERT INTO "+
                                " tn_student_list "+
                                "( sl_f_name, sl_m_name, sl_l_name, sl_mobile_number_parents, sl_contact_number ) "+
                                " VALUES (?)";
    ...
    getStudentTestPaper: function (pool, exam_details){// not in use
        return new Promise(function(resolve,reject){
            ...
            pool.query(query, [exam_details.test_id, exam_details.publish_id, exam_details.student_id],
                function (err, result){
                           if (err) {
    ...
    verifyLink: function (pool,link){
        return new Promise(function(resolve,reject){
            pool.query("SELECT * FROM tm_publish_test_list WHERE is_test_generated = 0 AND is_test_loaded = 1 AND ptl_link_1 = ?", [link], function (err, result) {
                if (err) {
                    reject(err);
                } else {
                    resolve(result); 
    ...
    addLockUser: function (pool, exam_details,callback) {
        ...
        pool.query(query, [exam_details.student_id],
    ...
    createStudentQuestionPaper: function(pool,exam_details){
        var _this = this;
        return new Promise(function (resolve, reject) {
                _this._getTestDataSet(pool, exam_details)
    ...
    _insertStudentToPaper: function (pool, exam_details){
        return new Promise(function(resolve,reject){
            ...
            var date = date1+' '+time;
            var query = "INSERT INTO tm_student_test_list "+
                        " (stl_test_id, stl_stud_id, stl_publish_id, stm_min, stm_sec,stl_time_stamp,stl_date,stl_time) "+
                        " VALUES ? ";
            pool.query(query,
                [[[exam_details.test_id, exam_details.student_id, exam_details.publish_id, exam_details.exam_time,0,date,date1,time]]],
    ...
    checkIsUserExist:function(pool,exam_details){
        return new Promise(function(resolve,reject){
            var query = " SELECT stl_test_status FROM tm_student_test_list  "+
                        " WHERE  stl_test_id = ? AND stl_stud_id = ? AND "+
                        " stl_publish_id = ? ";
            pool.query(query, [exam_details.test_id, exam_details.student_id, exam_details.publish_id],
                      function(err,result_data){
                        if (err) {
                            reject(err);
                        }else{
    ...
    deleteStudentTesList: function (pool, exam_details,callback) {
        var query = "DELETE FROM tm_student_question_paper "+
                    " WHERE sqp_test_id = ? AND sqp_student_id = ? AND " +
                    " sqp_publish_id = ?";
        pool.query(query, [exam_details.test_id, exam_details.student_id, exam_details.publish_id],
            function (err, result_data) {
                if (err) {
    ...
    updateTestQuestionDetails: function(pool,qD){
        return new Promise(function(resolve,reject){
            if (typeof (qD) == 'undefined') {
                ...
                var query = "UPDATE tm_student_question_paper SET "+
    ...
    updateStudentTestTimerAndStatus: function(pool,tD){
        return new Promise(function (resolve, reject) {
            var query = "UPDATE tm_student_test_list SET " +
    ...
    updateStudentExamStatus: function(pool,tD){
        return new Promise(function (resolve, reject) {
            var query = "UPDATE tm_student_test_list SET " +
                " stl_test_status = ?  WHERE  stl_stud_id = ? AND stl_publish_id = ?";
    ...
    getUserClimaxSelfDetailsRequest: function(pool,data){
        return new Promise(function(resolve, reject){
            var query ="SELECT sl_f_name as f_name,"+
				        " sl_m_name as m_name, "+
					    " sl_l_name as l_name, "+
					    " sl_image as s_image, "+
					    " sl_sign as s_sign, "+
    ...
    getUserClimaxExamDetailsRequest: function(pool, data){
        return new Promise((resolve, reject) => {
            var query = "SELECT main_topic_name as subject_name,"+
