...
module.exports = {
    ...
    getStudentTestPaper: function (pool, exam_details) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            pool.query(
                query,
                [exam_details.test_id, exam_details.publish_id, exam_details.student_id],
                function (err, result) {
                    if (err) {
    ...
    checkExamCreatedNot: function (pool, exam_details) {
        return new Promise(function (resolve, reject) {
            var query =
                'SELECT id FROM tm_student_test_list WHERE stl_publish_id = ? AND stl_stud_id = ?  LIMIT 1';
    ...
    markAsPresent: function (pool, exam_details, callback) {
        var query =
            'UPDATE tn_student_list SET ' + ' sl_present_status = 1 ' + ' WHERE ' + ' id = ?';
        ...
        return new Promise(function (resolve, reject) {
            _this
                ...
                .then(function (isTestAdded) {
                    return resultStatus.checkResultInserted(
                        isTestAdded,
                        'Agreement Faild Type 1,Try Again'
                    );
                })
                ...
                .then(function (isStudentAddedToTest) {
                    return resultStatus.checkResultInserted(
                        isStudentAddedToTest,
                        'Agreement Faild Type 2,Try Again'
                    );
                })
    ...
    _insertStudentTestPaper: function (pool, db_data) {
        return new Promise(function (resolve, reject) {
            if (db_data.length > 0) {
                var insert_array = [];
                var query =
                    'INSERT INTO tm_student_question_paper (sqp_question_id, sqp_student_id, sqp_publish_id, sqp_test_id) VALUES ?';
                db_data.forEach(function (value, index, main_array) {
                    insert_array.push([
                        value.q_id,
                        value.sqp_student_id,
                        value.sqp_publish_id,
                        value.test_id,
                    ]);
                });
    ...
    checkIsUserExist: function (pool, exam_details) {
        return new Promise(function (resolve, reject) {
            var query =
                ' SELECT stl_test_status FROM tm_student_test_list  ' +
    ...
    deleteStudentTesList: function (pool, exam_details, callback) {
        var query =
            'DELETE FROM tm_student_question_paper ' +
    ...
    updateTestQuestionDetailsV2: function (pool, qD) {
        return new Promise(function (resolve, reject) {
            if (typeof qD == 'undefined') {
                reject({ _call: 3 });
            } else {
