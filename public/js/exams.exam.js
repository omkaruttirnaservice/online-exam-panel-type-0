...
function getAllExamList() {
    $.get(getUrl() + "get-all-exam-list", function (data, status) {
        if (status == "success") {
            var _json = data;
            if (_json.call == 1) {
            ...
            if (type == 1) {
                ...
                if (value.ptl_is_test_done == "1") {
                    ...
                    if (value.is_test_loaded == "1") {
                        _tr +=
                            '<td><label  class="btn-success btn-sm" style="margin-right:10px;">' +
                            value.ptl_link_1 +
                            "</label>" +
                            "<td>" +
                            value.mt_test_time +
                        ...
                        _tr +=
                            '<td><center><label  class=""  title="' +
                            value.ptl_link_1 +
                            '"><i class="text-success fa fa-history fa-2x"></i><center></label>' +
                            "<td>" +
                            value.mt_test_time +
                            " Min</td>" +
                            "<td>Batch-" +
                ...
                if (value.is_test_loaded == "1") {
                    if (value.is_start_exam == "1") {
                        if (value.is_test_generated == "1") {
                            _tr +=
                                '<td><center><i class="text-success fa fa-check-square-o fa-2x"></i></center></td>' +
                            ...
                            _tr +=
                                '<td><center><i class="text-success fa fa-check-square-o fa-2x"></i></center></td>' +
                                '<td><button type="button" class="btn btn-primary btn-sm" onclick="takeExamBackup(' +
                                value.id +
                                "," +
                                value.tm_allow_to +
                                "," +
                                cc +
                                ')">End Slot & Backup</button></td>' +
                                '<td><center><i class="text-danger fa fa-hourglass-o fa-2x"></i></center></td>';
                        }
                    } else {
                        _tr +=
                            '<td><button type="button" class="btn btn-success btn-sm" onclick="activeExam(' +
                            value.id +
                    ...
                    _tr +=
                        '<td><button type="button" class="btn btn-primary btn-sm" onclick="activeSession(' +
                        value.id +
                        ')">Start Session</button></td>' +
                        '<td><center><i class="text-danger fa fa-hourglass-o fa-2x"></i></center></td>' +
                        '<td><center><i class="text-danger fa fa-hourglass-o fa-2x"></i></center></td>';
                }
                if (value.is_start_exam == "1") {
                    if (value.is_absent_mark == "1") {
                        _tr +=
                            '<td><center><i class="text-danger fa fa-check-circle fa-2x"></i></center></td></tr>';
                    } else {
                        _tr +=
                            '<td><center><button type="button" class="btn btn-danger btn-sm" style="margin-right:10px;" onclick=markStudentAsAbsent(this,event,' +
                            value.id +
                            "," +
                            value.tm_allow_to +
                            ")>Mark Batch-" +
                            value.tm_allow_to +
                            " Absent </button></center></td></tr>";
                    }
                } else {
                    _tr +=
                        '<td><center><i class="text-danger fa fa-hourglass-o fa-2x"></i></center></td></tr>';
                }
                /* if(value.is_test_loaded == '1'){
                ...
                _tr +=
                    '<td><button type="button" class="btn btn-primary btn-sm" onclick="downloadExam(' +
                    value.id +
                    "," +
                    index +
                    ')">Download</button></td>' +
    ...
    prompt(
        ...
        function (value) {
            ...
            $.post(
                    ...
                    if (status == "success") {
                        ...
                        switch (_json.call) {
                            ...
                            case 1:
                                ...
                                getAllExamList();
                                new_exam_list = new_exam_list.filter(
                                    function (ele) {
                                        return ele.id != id;
                                    }
                                );
                ...
                if (error.status === 404) {
                    alert(errJSON?.errMsg || "404 error");
                } else {
                    alert(errJSON?.errMsg || "Server Error");
                }
            });
        }
    );
...
function downloadStudent(pub_id, student_type) {
    $.post(
        getUrl() + "download-student",
        { student_type: student_type, pub_id: pub_id },
        function (data, status) {
            if (status == "success") {
                var _json = data;
                switch (_json.call) {
...
function takeExamBackup(pub_id, batch_no, cc) {
    confirmCall(
        "Is Scheduled Exam Completed ?",
        function ($this_data, type, ev) {
            if (type == "yes") {
                confirmCall(
                    "Are You Sure To Create Backup ?",
                    function ($this_data, type, ev) {
                        if (type == "yes") {
                            prompt(
                                "Enter Authentication Password",
                                        ...
                                        function (data, status) {
                                            ...
                                            if (status == "success") {
                                                ...
                                                switch (_json.call) {
                                                    ...
                                                    case 2:
                                                        alert(
                                                            "Student Records Not Found For Backup."
                                                        );
                                                        break;
                                                    default:
                                                        alert(
